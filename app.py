...
books_collection = client['mylibrary']["books"]
reviews_collection = client['mylibrary']["reviews"]
user_library_collection = client['mylibrary']["user_library"]
...
def _enrich_library_items(items: list) -> list:
    """Join user library items with book details and merge fields."""
    enriched = []
    for it in items:
        book_id = it.get('bookId')
        try:
            b = books_collection.find_one({'_id': ObjectId(book_id)}) if book_id else None
        except Exception:
            b = None
        base = serialize_book(b) if b else {"bookId": book_id}
        base.update({
            'isFavorite': bool(it.get('isFavorite', False)),
            'userRating': it.get('rating', None),
...
@app.route('/user/<user_id>/library', methods=['GET'])
@cross_origin(origin='http://localhost:4200', supports_credentials=True)
def get_user_library(user_id):
    try:
        items = list(user_library_collection.find({'userId': user_id}))
        return jsonify(_enrich_library_items(items)), 200
    except Exception as e:
        return jsonify({'error': 'Failed to fetch library'}), 500


@app.route('/user/<user_id>/library', methods=['POST'])
@cross_origin(origin='http://localhost:4200', supports_credentials=True)
def add_to_user_library(user_id):
    data = request.get_json() or {}
...
@app.route('/user/<user_id>/library/<book_id>', methods=['PUT'])
@cross_origin(origin='http://localhost:4200', supports_credentials=True)
def update_user_library_item(user_id, book_id):
    data = request.get_json() or {}
    update_doc = {}
    if 'isFavorite' in data:
        update_doc['isFavorite'] = bool(data['isFavorite'])
    if 'rating' in data:
        update_doc['rating'] = data['rating']
    if not update_doc:
        return jsonify({'error': 'No valid fields to update'}), 400
    res = user_library_collection.update_one({'userId': user_id, 'bookId': book_id}, {'$set': update_doc})
    if res.matched_count == 0:
        return jsonify({'error': 'Library item not found'}), 404
    item = user_library_collection.find_one({'userId': user_id, 'bookId': book_id})
    return jsonify(_enrich_library_items([item])[0]), 200


@app.route('/user/<user_id>/library/<book_id>', methods=['DELETE'])
@cross_origin(origin='http://localhost:4200', supports_credentials=True)
def remove_user_library_item(user_id, book_id):
    res = user_library_collection.delete_one({'userId': user_id, 'bookId': book_id})
...
def get_user_favorites(user_id):
    try:
        items = list(user_library_collection.find({'userId': user_id, 'isFavorite': True}))
        return jsonify(_enrich_library_items(items)), 200
    except Exception as e:
